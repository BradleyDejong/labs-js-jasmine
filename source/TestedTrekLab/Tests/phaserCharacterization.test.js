...
import { Weapon } from "../StarTrek/Weapon";
...
describe("phasers", function() {
  ...
  it("should complain with insufficient energy when not available", function() {
    ...
    // then
    expect(ui.writeLine).toHaveBeenCalledWith(
      "Insufficient energy to fire phasers!"
    );
  });
  ...
  describe("when Klingon out of range", function() {
    ...
    beforeEach(function() {
      outOfRange = Phasers.prototype.maxPhaserRange + 1;
    ...
    it("reports out-of-range", function() {
      expect(ui.writeLine).toHaveBeenCalledWith(
        "Klingon out of range of phasers at " + outOfRange + " sectors..."
      );
    });
  ...
  describe("when damaging Klingon", function() {
    ...
    it("reports damage", function() {
      expect(ui.writeLine).toHaveBeenCalledWith(
        "Phasers hit Klingon at 2000 sectors with 25 units"
      );
